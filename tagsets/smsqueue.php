...
function smsqueue__show_smsqueue($experiment_id="",$limit=-1) {
...
if ($proceed) {
    if ( $dall || $dallpage || $dsel ) {
        ...
        if ($experiment_id) $allow=check_allow('mailqueue_edit_experiment','experiment_smsqueue_show?experiment_id='.$experiment_id);
        else $allow=check_allow('mailqueue_edit_all','mailqueue_show.php');

        $where_clause=" WHERE sms_id IS NOT NULL ".$equery;
        ...
        if ($dallpage) {
            ...
            if (count($tallids)>0) {
                ...
                $where_clause.=" AND sms_id IN (".implode(",",$parnames).") ";
                $ok=true;
            } else {
        ...
        if ($ok) {
            $query="DELETE FROM ".table('sms_queue').$where_clause;
            //echo $query;
        ...
        if ($experiment_id) {
            redirect ("admin/experiment_smsqueue_show.php?experiment_id=".$experiment_id);
        } else {
            redirect ("admin/smsqueue_show.php");
        }
    }
}
...
if ($proceed) {
    ...
    if ($experiment_id && check_allow('mailqueue_edit_experiment')) {
        echo '<FORM action="experiment_smsqueue_show.php" method="POST">
            <INPUT type="hidden" name="experiment_id" value="'.$experiment_id.'">';
    } elseif (check_allow('mailqueue_edit_all')) {
        echo '<FORM action="smsqueue_show.php" method="POST">';
    }
